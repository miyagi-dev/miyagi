...
Since _miyagi_ is a lot about developing components independently of a backend framework, we might also want to mock API endpoints, for example to test form submissions using AJAX requests.

We can do that easily using [MSW](https://mswjs.io).

Please follow the instructions on the _MSW_ website to setup MSW using _Browser Integration_ (not the Node integration). When generating the worker script, we can use whatever directory you like for the `PUBLIC_DIR`. We will for now use the current directory, so `.`. When we are done with the setup, we will have with the following files:
