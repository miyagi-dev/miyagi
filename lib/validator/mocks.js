 ...
 */
module.exports = function validateMockData(component, dataArray, noCli) {
		...
		if (validate) {
			dataArray.forEach((entry) => {
				...
				if (!valid && !noCli) {
					log(
						"error",
						`${component.paths.dir.short} — ${entry.name}: ${validate.errors
							.map(
								(error) =>
									`${error.message} (${Object.entries(error.params).map(
	...
	if (!global.config.isBuild && !noCli) {
		log(
			"warn",
			`${component.paths.dir.short}: ${config.messages.validator.mocks.noSchemaFound}`
		);
	}
