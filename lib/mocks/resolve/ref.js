	...
	if (entry) {
		...
		if (entry instanceof Array) {
			...
			for (const ent of entry) {
				const newComponent = getNewComponentContext(ent.$ref, component);
		...
		for (const [key, value] of Object.entries({ ...entry })) {
			if (key === "$ref") {
				const newComponent = getNewComponentContext(value, component);
...
async function resolve(entry, component) {
	...
	if (entry !== null) {
		...
		if (entry.$ref) {
			...
			const newComponent = getNewComponentContext(entry.$ref, component);
 ...
 * @returns {object}
 */
function getNewComponentContext(value, currentComponent) {
	if (value && value.includes("#/$defs/") && !value.startsWith("#/$defs/")) {
		return global.state.routes.find(
			(route) => route.paths.dir.short === value.split("#")[0],
		);
	}
