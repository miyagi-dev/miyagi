 ...
 */
export default async function resolveData(data, component, rootData) {
	let merged = rootData ? mergeRootDataWithVariationData(rootData, data) : data;
	let resolved;
	merged = mergeWithGlobalData(merged);
	resolved = await overwriteJsonLinksWithJsonData({ ...merged }, component);
 ...
 */
async function overwriteJsonLinksWithJsonData(data, component) {
	return new Promise((resolve) => resolveRefs(data, component).then(resolve));
}
							...
							await global.app.render(
								...
								(err, html) => {
									if (err)
										log(
											"warn",
											t("renderingTemplateFailed").replace(
												"{{filePath}}",
