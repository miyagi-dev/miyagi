...
module.exports =
	 ...
	 */
	async function resolveData(data, rootData) {
		let merged = rootData
	...
	if (entry) {
		...
		await Promise.all(
			Object.entries({ ...entry }).map(async ([key, value]) => {
				if (key === "$ref") {
					o = { ...o, ...(await getRootOrVariantDataOfReference(value)) };
					delete o.$ref;
				} else {
					...
					fs.stat(component.paths.tpl.full, async function (err) {
						if (err == null) {
							...
							await global.app.render(
								path.relative("components", component.paths.tpl.short),
								getDataForRenderFunction(data),
								(err, html) => {
									if (err)
										log(
											"warn",
											t("renderingTemplateFailed")
												.replace("{{filePath}}", component.paths.tpl.short)
												.replace("{{engine}}", global.config.engine.name)
										);
