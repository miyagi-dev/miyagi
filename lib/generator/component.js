...
module.exports = async function componentGenerator({ component, fileTypes }) {
	global.config = await updateConfigForComponentGeneratorIfNecessary(
		global.config,
		fileTypes
	);
	...
	return new Promise((resolve, reject) => {
		createComponentFolder(component)
			.then(async () => {
				...
				resolve(
					messages.generator.component.done.replace("{{component}}", component)
				);
			})
			.catch((err) => {
				reject(err);
 ...
 */
async function updateConfigForComponentGeneratorIfNecessary(config, fileTypes) {
	if (!config.files.templates.extension && fileTypes.includes("tpl")) {
		if (config.engine && config.engine.name) {
			config = helpers.updateConfigWithGuessedExtensionBasedOnEngine(config);
		} else {
			config =
				await helpers.updateConfigWithGuessedEngineAndExtensionBasedOnFiles(
					config
				);
		}
	}
