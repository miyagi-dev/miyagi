...
module.exports = () => {
	return new Promise((resolve, reject) => {
		...
		fs.rm(path.resolve(buildFolder), { recursive: true }, () => {
			fs.mkdir(path.resolve(buildFolder), { recursive: true }, () => {
				...
				promises.push(
					new Promise((res, rej) => {
						buildDesignTokens(buildFolder, buildDate, formattedBuildDate)
							.then(() => {
								res();
							})
							.catch(rej);
				...
				global.state.routes.forEach((route) => {
					if (route.type === "docs") {
						promises.push(
							new Promise((res, rej) => {
								buildDoc(route, buildFolder, buildDate, formattedBuildDate)
									.then(res)
									.catch(rej);
							})
						);
	 ...
	 * @returns {Promise} gets resolved when the view has been rendered
	 */
	function buildDesignTokens(buildFolder, buildDate, formattedBuildDate) {
		const promises = [];
					...
					render.renderMainDesignTokens({
						...
						cb: (err, response) => {
							if (err) {
								...
								fs.writeFile(
									path.resolve(`${buildFolder}/design-tokens-${type}.html`),
									response,
									(err) => {
										if (err) {
											if (typeof err === "string") {
	 ...
	 * @param {string} formattedBuildDate
	 * @returns {Promise}
	 */
	function buildDoc(doc, buildFolder, buildDate, formattedBuildDate) {
