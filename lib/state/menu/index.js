...
function getComponentFiles(directory) {
	return directory.children.filter((child) => {
		...
		if (helpers.fileIsTemplateFile(baseName)) return true;
...
function getMenu() {
	...
	const docsMenu = getDocsMenu(global.state.sourceTree.docs);

	if (componentsMenu && docsMenu) {
		return [componentsMenu, docsMenu];
	} else if (componentsMenu) {
