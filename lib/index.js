...
function getCliArgs(args) {
  const cliArgs = Object.assign({}, args);
  ...
  if (cliArgs.assets) {
    if (cliArgs.assets.es6Modules) {
      cliArgs.assets.es6Modules =
        cliArgs.assets.es6Modules == "false" ? false : true;
    }
  }

  if (cliArgs.ui) {
    if (cliArgs.ui.reload) {
      cliArgs.ui.reload = cliArgs.ui.reload == "false" ? false : true;
    }

    if (cliArgs.ui.validations) {
      if (cliArgs.ui.validations.accessibility) {
        cliArgs.ui.validations.accessibility =
          cliArgs.ui.validations.accessibility == "false" ? false : true;
      }

      if (cliArgs.ui.validations.html) {
        cliArgs.ui.validations.html =
          cliArgs.ui.validations.html == "false" ? false : true;
      }
    }
  }
  ...
  if (isBuild || isGenerator || isServer || isDataGenerator) {
    ...
    if (isDataGenerator) {
      ...
      if (
        ...
        if (config.components && config.components.folder === undefined) {
