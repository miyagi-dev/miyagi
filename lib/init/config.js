...
function getJsFilesArray(strOrArrOrObj, manifest, root) {
	...
	if (typeof files === "string") {
		...
		if (nodeEnv && files[nodeEnv]) {
			...
			log(
				"warn",
				t("nodeEnvAndKeysDontMatchCssOrJs")
...
function getAssetFoldersArray(strOrArrOrObj) {
	...
	if (typeof folders === "string") {
		...
		if (nodeEnv && folders[nodeEnv]) {
			...
			log(
				"warn",
				t("nodeEnvAndKeysDontMatchAssetFolders").replace(
					"{{nodeEnv}}",
					nodeEnv,
				),
			);
...
export default (userConfig = {}) => {
	...
	if (config.ui.theme.logo) {
		if (typeof config.ui.theme.logo === "string") {
			...
			} else if (config.zu.theme.logo.dark && !config.ui.theme.logo.light) {
