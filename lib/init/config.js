...
module.exports = (userConfig = {}) => {
  if (userConfig.assetFolder) {
    userConfig.assetFolder = arrayfy(userConfig.assetFolder).map(
      sanitizeDirectory
    );
  }
  ...
  if (userConfig.files.templates.namespaces) {
    Object.entries(userConfig.files.templates.namespaces).forEach(
      ([key, val]) => {
        userConfig.files.templates.namespaces[key] = path.join(
          process.cwd(),
