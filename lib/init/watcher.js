...
async function updateData(appInstance, events) {
  const data = cloneDeep(appInstance.get("state").data);
  const promises = [];
      ...
      if (event === "add" || event === "change") {
        promises.push(
          new Promise(async (resolve) => {
            data[changedPath] = await readFile(changedPath);
            resolve();
          })
        );
...
async function handleFileChange() {
  if (triggeredEventsIncludes(triggeredEvents, ["addDir", "unlinkDir"])) {
    await setState(appInstance, {
      sourceTree: true,
      data: await updateData(appInstance, triggeredEvents),
      menu: true,
      partials: true,
    });
