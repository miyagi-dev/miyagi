...
function changeFileCallback(reloadParent) {
  if (appInstance.get("config").ui.reload) {
...
async function handleFileChange() {
  if (triggeredEventsIncludes(triggeredEvents, ["addDir", "unlinkDir"])) {
    ...
    helpers.fileIsDataFile(appInstance, triggeredEvents[0].changedPath)
  ) {
    await setState(appInstance, {
      fileContents: await updateFileContents(appInstance, triggeredEvents),
    ...
    helpers.fileIsDocumentationFile(appInstance, triggeredEvents[0].changedPath)
  ) {
    await setState(appInstance, {
      fileContents: await updateFileContents(appInstance, triggeredEvents),
    });
    ...
    helpers.fileIsInfoFile(appInstance, triggeredEvents[0].changedPath)
  ) {
    await setState(appInstance, {
      fileContents: await updateFileContents(appInstance, triggeredEvents),
      menu: true,
    ...
    helpers.fileIsSchemaFile(appInstance, triggeredEvents[0].changedPath)
  ) {
    await setState(appInstance, {
      fileContents: await updateFileContents(appInstance, triggeredEvents),
    });
...
module.exports = function(server, app) {
  ...
  const { components } = appInstance.get("config");
  const ignored = getIgnoredPathsArr(components.ignores);

  chokidar
    .watch(path.resolve(components.folder), {
      ignoreInitial: true,
      ignored,
    })
