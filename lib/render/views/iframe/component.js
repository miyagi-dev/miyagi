...
export default async function renderIframeComponent({
	...
	if (componentJson?.length > 0) {
		...
		const componentData = hasTemplate
			? await getVariationData(component)
			: { raw: {}, extended: {} };
	...
	if (templateFilePath) {
		if (context.length > 0) {
			...
			if (!renderInIframe) {
				promises.push(
					new Promise((resolve, reject) => {
						global.app.render(templateFilePath, {}, (err, result) => {
							if (err) {
								if (typeof err === "string") {
									log("error", err);
								} else if (err.message) {
							...
							variations[0] = {
								...getData("default", component.paths.dir.short, {}, []),
								html: result,
								error: err,
							};
...
function getData(variation, shortPath, entry, validatedMocks) {
	...
	const data = {
		...
		standaloneUrl,
		mockData: JSON.stringify(entry.rawData),
		mockDataResolved: JSON.stringify(entry.data),
	};
