...
async function resolveData(app, data, rootData) {
  ...
  data = await overwriteAttributesKey(app, data);
...
function getFallbackData(variations, rootData) {
  for (let i = 0; i < variations.length; i += 1) {
    if (variations[i].data) {
      if (rootData) {
        return { ...rootData, ...variations[i].data };
      }
...
function overwriteAttributesKey(app, data) {
  ...
  if (data instanceof Array) {
    obj = data.map((entry) => overwriteAttributesKey(app, entry));
  } else {
    obj = { ...data };
    ...
    if (obj) {
      ...
      for (const [key, val] of entries) {
        if (key === "attributes()") {
          ...
          obj[key] = overwriteAttributesKey(app, val);
        }
      }
    }
  }
