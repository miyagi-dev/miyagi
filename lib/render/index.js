...
async function renderComponentVariations({ app, res, file, embedded, cb }) {
  ...
  if (componentVariations) {
    ...
    await Promise.all(
      ...
      await renderVariations({ app, res, file, context, cb });
    });
  } else {
    await renderSingleComponent({
      app,
...
async function renderVariations({ app, res, file, context, cb }) {
  const variations = [];
  const promises = [];
