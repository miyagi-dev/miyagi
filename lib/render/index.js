...
async function renderComponentVariations({ app, res, file, cb }) {
  ...
  let componentName = fileName.replace(`.${app.get("config").extension}`, "");
  let componentStatus;
        ...
        context.map(async (entry, i) => {
          return new Promise(async (resolve) => {
            context[i].data = mergeWithGlobalData(
              app,
              await await overwriteTplLinksWithTplContent(
                app,
                overwriteJsonLinksWithJsonData(app, entry.data)
              )
            );
            resolve();
          });
  ...
  const components = Object.keys(app.get("state").partials).map(
    (partialPath) => {
      ...
      return [
        ...
        componentJson.name ||
          path.basename(partialPath, `.${app.get("config").extension}`),
