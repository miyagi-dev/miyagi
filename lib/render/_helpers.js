...
function getRootOrVariantData(app, value) {
  ...
  const val = `${shortVal}/${path.basename(shortVal)}.json`;
    ...
    if (entry) {
      if (Array.isArray(entry)) {
        ...
        return Promise.all(promises).then(() => {
          const filePath = `${entry.$tpl}/${entry.$tpl
            .split(path.sep)
            .slice(-1)}.${app.get("config").templates.extension}`;
          app.render(
            filePath,
              ...
              if (err)
                logger.log(
                  ...
                  config.messages.renderingTemplateFailed
                    ...
                    .replace("${engine}", app.get("config").templates.engine)
                );
              resolve1(html);
            }
          );
...
function getDataForRenderFunction(app, data) {
  ...
  return Object.assign({}, data, {
    ...
    namespaces: app.get("config").templates.namespaces, // for twig
